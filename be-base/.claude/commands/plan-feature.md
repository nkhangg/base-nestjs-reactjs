Plan a new feature for an existing domain module. Produces a detailed implementation plan without writing any code.
...
Based on the feature description and module doc, determine which core docs are relevant:
...
- If touching **entity**: read `domain/entities/<module>.entity.ts`
- If touching **repository interface**: read `domain/repositories/<module>.repository.ts`
- If touching **use-case**: read the most similar existing use-case
- If touching **controller**: read the existing controller
- If touching **Prisma repo**: read `infrastructure/repositories/prisma-<module>.repository.ts`
- If touching **mapper**: read `infrastructure/mappers/<module>.mapper.ts`
...
Files:
  - CREATE  src/modules/<module>/domain/...   — <what>
  - MODIFY  src/modules/<module>/domain/...   — <what>
  (or "No changes needed")

### Application
<Describe new or modified use-cases>
Files:
  - CREATE  src/modules/<module>/application/use-cases/...   — <what>
  - MODIFY  src/modules/<module>/application/use-cases/...   — <what>
  (or "No changes needed")

### Infrastructure
<Describe Prisma repo changes, mapper changes, in-memory repo changes>
Files:
  - MODIFY  src/modules/<module>/infrastructure/...   — <what>
  (or "No changes needed")

### Presentation
<Describe new routes, DTO changes, controller changes>
Files:
  - MODIFY  src/modules/<module>/presentation/...   — <what>
