...
export function ForgotPasswordPage() {
      ...
      <div className="flex min-h-svh items-center justify-center bg-black px-4">
        <div className="w-full max-w-sm rounded-2xl bg-zinc-900 p-8 shadow-xl">
          <div className="mb-6 flex flex-col items-center gap-3 text-center">
            ...
            <h1 className="text-lg font-semibold text-white">{t('auth.checkEmail')}</h1>
            <p className="text-sm text-zinc-400">{t('auth.emailSentMsg')}</p>
          </div>
  ...
  return (
    <div className="flex min-h-svh items-center justify-center bg-black px-4">
      <div className="w-full max-w-sm rounded-2xl bg-zinc-900 p-8 shadow-xl">
        <div className="mb-6">
          <h1 className="text-xl font-semibold text-white">{t('auth.forgotTitle')}</h1>
          <p className="mt-1 text-sm text-zinc-400">{t('auth.forgotSubtitle')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          >
            {isPending ? t('auth.processing') : t('auth.sendRequest')}
          </button>
        </form>
